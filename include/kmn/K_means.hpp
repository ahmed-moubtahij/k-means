...
template <typename T, size_type D> struct distance_from {
    ...
    template <typename U>
    constexpr auto operator()(DataPoint<U, D> const& c1,
...
using centroid_t = select_centroid_t<point_value_t<PTS_R>,
                                     data_point_size_v<data_point_t<PTS_R>>>;

template <typename PTS_R>
using indexed_centroids_t = vector<std::pair<size_type, centroid_t<PTS_R>>>;
...
auto init_centroids(PTS_R&& data_points, size_type k)
    ...
    if constexpr (std::floating_point<pt_value_t>) {
        auto const centroids = FWD(data_points) | sample(k) | to<centroids_t>();
...
void update_centroids(auto&& data_points, auto&& out_indices,
    ...
    auto constexpr mean_matching_points = [](auto&& points) {
        ...
        auto const counted_sum = [&count](data_point_t const& pt1,
                                          data_point_t const& pt2) {
            ++count;
            return pt1 + pt2;
        };
        // count is a side effect of counted_sum,
        // so sum must be calculated separately
        auto const sum = std::reduce(stdr::begin(points), stdr::end(points),
    ...
    stdr::transform(r3v::keys(indexed_centroids),
                    ...
                    [&](auto const& cent_id) {
                        return mean_matching_points(
                            ...
                            r3v::filter(match_id{cent_id}) | r3v::values);
                    });
}

void index_points_by_centroids(auto&& out_indices, auto&& data_points,
                               auto const& indexed_centroids) {
    auto constexpr dist_from_centroid = [](auto const& pt) {
        return distance_from{pt};
    };

    auto constexpr proj_centroid = [](auto const& id_pt) {
        return id_pt.second;
    };
    ...
    auto const find_id_nearest_centroid =
        [&](auto const& pt) { // stdr::borrowed_iterator has no operator->
            return (*stdr::min_element(indexed_centroids,
                                       dist_from_centroid(pt), proj_centroid))
                .first;
        };
    // Find the ID of the nearest centroid to each data point,
    // and write it to the output range
    stdr::transform(FWD(data_points), std::begin(FWD(out_indices)),
...
template <typename CENTROIDS_R, typename SIZES_R, typename INPUT_R,
          typename OUTPUT_R>
struct k_means_result {
    ...
    using filtered_indexed_range =
        decltype(declval<indexed_range>() | r3v::filter(match_id{}));

    using filtered_range =
        decltype(declval<filtered_indexed_range>() | r3v::values);
        ...
        auto operator*() -> cluster {
            return {parent.centroids[cluster_idx],
                    zip(FWD(parent.out_indices), FWD(parent.points)) |
                        r3v::filter(match_id{cluster_idx + 1}) | r3v::values};
        }
...
template <typename PTS_R, typename IDX_R>
constexpr auto k_means_impl(PTS_R&& data_points, IDX_R&& out_indices,
                            size_type k, size_type n)
    -> k_means_impl_t<PTS_R, IDX_R> { // Initialize centroids and their ids
    ...
    return {r3v::values(indexed_centroids) |
            ...
            clusters_histogram(FWD(out_indices), k), FWD(data_points),
            FWD(out_indices)};
}
...
struct k_means_fn {
    ...
    constexpr auto
    operator()(PTS_R&& data_points, // Not mutated but a reference to it is
