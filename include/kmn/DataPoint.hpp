...
struct DataPoint final : private std::array<T, D> {
    ...
    // Hidden -i.e. only found through ADL- friends are preferred for symmetric
    // operations
    friend constexpr auto operator+(DataPoint const& lhs, DataPoint const& rhs)
        -> DataPoint {
        DataPoint res;
    ...
    // operator/ overload for floating point value type; result's value type
    // matches it
    constexpr auto operator/(arithmetic auto n) const
        -> DataPoint<value_type, D> requires(std::floating_point<value_type>) {
    ...
    constexpr auto operator/(arithmetic auto n) const -> DataPoint<double, D> {
